  ...
  .controller('MainCtrl', function ($scope, $http) {
    ...
    $scope.groupsMatches = {
        ...
        C: { matches:[
            ...
            [{country: "ivorycoast", score:0}, {country: "japan", score:0}],
            [{country: "colombia", score:0}, {country: "ivorycoast", score:0}],
            [{country: "japan", score:0}, {country: "greece", score:0}],
            [{country: "japan", score:0}, {country: "colombia", score:0}],
            [{country: "greece", score:0}, {country: "ivorycoast", score:0}]
        ],
            standing: {
                colombia: 0,
                ivorycoast:0,
        ...
        F: { matches:[
            [{country: "argentina", score:0}, {country: "bosniaandherzegovina", score:0}],
            [{country: "iran", score:0}, {country: "nigeria", score:0}],
            [{country: "argentina", score:0}, {country: "iran", score:0}],
            [{country: "nigeria", score:0}, {country: "bosniaandherzegovina", score:0}],
            [{country: "nigeria", score:0}, {country: "argentina", score:0}],
            [{country: "bosniaandherzegovina", score:0}, {country: "iran", score:0}]
        ],
            standing: {
                argentina: 0,
                iran:0,
        ...
        G: { matches:[
            ...
            [{country: "ghana", score:0}, {country: "unitedstates", score:0}],
            [{country: "germany", score:0}, {country: "ghana", score:0}],
            [{country: "unitedstates", score:0}, {country: "portugal", score:0}],
            [{country: "unitedstates", score:0}, {country: "germany", score:0}],
        ...
        H: { matches:[
            ...
            [{country: "russia", score:0}, {country: "southkorea", score:0}],
            [{country: "belgium", score:0}, {country: "russia", score:0}],
            [{country: "southkorea", score:0}, {country: "algeria", score:0}],
            [{country: "southkorea", score:0}, {country: "belgium", score:0}],
    ...
    $scope.updateResuts = function() {
        $scope.matchesLoaded = false;
        $http.get('/api/scraper/matches').success(function(matches) {
            for (var i = 0; i < matches.length; i++) {
                var matchResult = matches[i];
                var index = 0;
                var match = _.find($scope.groupsMatches, function(group){
                    for (var i = 0; i < group.matches.length; i++) {
                         var match = group.matches[i];
                         if(match[0].country.toLowerCase() == matchResult.homeTeam.replace(/ /g,'').toLowerCase() &&
                            match[1].country.toLowerCase() == matchResult.awayTeam.replace(/ /g,'').toLowerCase()) {
                            index = i;
                            return true;
                         }
                     }
                    });
                match.matches[index][0].score = matchResult.result[0] ? matchResult.result[0] : 0;
                match.matches[index][0].fullCountryName = matchResult.homeTeam;
                match.matches[index][1].score = matchResult.result[1] ? matchResult.result[1] : 0;
                match.matches[index][1].fullCountryName = matchResult.awayTeam;
            }
    ...
    $scope.$watch("matchesLoaded", function(newVal) {
        if(newVal) {
            loadingModal.hide();
        } else {
            loadingModal.show();
    ...
    setTimeout(function(){ $scope.updateResuts(); }, 500);
  });
